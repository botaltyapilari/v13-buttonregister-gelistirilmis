...
module.exports = async member => {
    ...
    if (kurulus > 604800000) {
        ...
        member.guild.channels.cache.get(config.channels.welcomechannel).send(`**Jose**'ye hoş geldin ${member}. Seninle sunucumuz **${member.guild.memberCount}** üyeye ulaştı!
        
Hesabın **${kuruluş}** tarihinde (**${zaman} önce**) oluşturulmuş **|** Güvenli! <a:aden_green:1005225223416729720>
Soldaki ses teyit odalarından birine girerek <@&${config.registration.staff}> rolüne sahip yetkililerimize teyit verip kayıt olabilirsin!

Tagımıza ulaşmak için herhangi bir kanala **.tag** yazabilirsiniz.

Sunucu kuralları <#1005227879996608602> kanalında belirtilmiştir. Tüm cezai işlemler kuralları **okuduğun varsayılarak** yapılacaktır! `);
    } else {
        member.setNickname(config.registration.susoeciosnickname);
        member.roles.add(config.registration.suspecios);
        member.setNickname(config.registration.autonickname);
        member.guild.channels.cache.get(config.channels.welcomechannel).send(`${member} adlı kullanıcı sunucuya katıldı, hesabı **${zaman2}** önce açıldığı için şüpheli.\n **|** Güvenilir Değil! <a:aden_red:1005225225379659816>`)
    }
}
