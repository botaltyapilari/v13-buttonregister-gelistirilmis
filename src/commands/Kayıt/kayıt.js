...
module.exports = {
    ...
    .setCustomId("erkek")
    .setStyle("SECONDARY")
    .setEmoji ("<:aden_erkek:1005446251095003187>")
    ...
    .setCustomId("kadin")
    .setStyle("SECONDARY")
    .setEmoji("<:aden_kiz:1005446278353780838>")

    var Bb = new MessageButton()
    .setLabel("İptal")
    .setCustomId("iptal")
    .setStyle("DANGER")
    .setEmoji("<a:aden_red:1005225225379659816>")
...
${member.toString()} kullanıcısını cinsiyetini seçerek kayıt edebilirsiniz.

İşlemi gerçekleştimek için 30 saniyen var.

.isimler komutunu kullanarak kullanıcının kayıtlı isimlerini görüntüleyebilirsin!
`)
    ...
    collector.on("collect", async (button) => {
      ...
      if(button.customId === "erkek") {
        ...
        client.channels.cache.get(config.channels.chat).send(`${config.emojis.tada} ${member} aramıza katıldı, Hoşgeldinn!!`).then((e) => setTimeout(() => { e.delete(); }, 15000));
